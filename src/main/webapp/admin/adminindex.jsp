    ...
    $.ajax({
		...
		success:function(back){
			...
			for(var i = 0;i < jack.length;i++){
				if(jack[i].parentModelId == 1){
					str += '<li class="layui-nav-item"><a  href="javascript:;" ><i class="layui-icon '+jack[i].modelImg+'" style="font-size:16px;"></i> <span style="margin-left:10px;font-weight:bold;">'+jack[i].title+'</span></a><dl class="layui-nav-child">';
					for(var j = 0;j < jack.length;j++){
						if(jack[i].id == jack[j].parentModelId){
							str +='<dd><a href="javascript:;"  name="'+jack[j].modelUrl+'" class="'+jack[j].title+'"> <span >'+jack[j].title+'</span></a></dd>';
						}
					}
				}
	            str +='</dl></li>';        
